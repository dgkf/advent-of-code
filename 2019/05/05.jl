...
mem_access(m, i, imm) = (imm ? i : m[i])+1 > length(m.vec) ? 0 : imm ? m[i] : m[m[i]]
...
op!(c::Val{1}, m, i, ps::Array{Int}) = (i+4, setindex!(m, ps[1] + ps[2], m[i+3]))
op!(c::Val{2}, m, i, ps::Array{Int}) = (i+4, setindex!(m, ps[1] * ps[2], m[i+3]))
op!(c::Val{3}, m, i, ps::Array{Int}) = (i+2, setindex!(m, popfirst!(op_stdin), m[i+1]))
op!(c::Val{4}, m, i, ps::Array{Int}) = (i+2, push!(op_stdout, ps[1]))
op!(c::Val{5}, m, i, ps::Array{Int}) = (ps[1] != 0 ? ps[2] : i+3, nothing)
op!(c::Val{6}, m, i, ps::Array{Int}) = (ps[1] == 0 ? ps[2] : i+3, nothing)
op!(c::Val{7}, m, i, ps::Array{Int}) = (i+4, setindex!(m, ps[1] < ps[2], m[i+3]))
op!(c::Val{8}, m, i, ps::Array{Int}) = (i+4, setindex!(m, ps[1] == ps[2], m[i+3]))
