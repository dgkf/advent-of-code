...
# part 1
Δinput = diff(input)
println(sum(Δinput .== 1) * sum(Δinput .== 3))
...
println(@time prod(map(count_paths, splitmap(vcat(false, diff(input) .== 3), input))))
...
println(@time count_paths_memoize(input))
...
println(@time count_paths_memoizejl(input))
...
println(@time count_paths_memoizationjl(input))
